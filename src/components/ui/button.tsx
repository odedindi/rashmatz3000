...
const ButtonLoaderIconWrapper: React.FC<
  React.PropsWithChildren<{
    loading: boolean;
    leftSection?: boolean;
    rightSection?: boolean;
  }>
> = ({ children, loading, leftSection, rightSection }) => (
  <>
    {(leftSection || (!leftSection && !rightSection)) && loading ? (
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const showLeftSection = leftSection && !loading;
    const showRightSection = rightSection && !loading;
    return (
      <Comp
        ...
        {...props}
      >
        <ButtonLoaderIconWrapper
          leftSection={!!leftSection}
          rightSection={!!rightSection}
          loading={loading}
        >
          {showLeftSection ? <div className="mr-2">{leftSection}</div> : null}
          {children}
          {showRightSection ? <div className="ml-2">{rightSection}</div> : null}
        </ButtonLoaderIconWrapper>
      </Comp>
    );
  }
